...
from .utils import ErrorSanitizer, boolean_string
...
class ClickhouseCheck(AgentCheck):
    ...
    def query_system_replicas(self):
        # https://clickhouse.yandex/docs/en/operations/system_tables/#system_tables-replicas
        for replica in self.execute_query_defined_schema(queries.SystemReplicas):
            tags = [
                'database:{}'.format(replica['database']),
                'table:{}'.format(replica['table']),
                'is_leader:{}'.format(boolean_string(replica['is_leader'])),
                'is_readonly:{}'.format(boolean_string(replica['is_readonly'])),
                'is_session_expired:{}'.format(boolean_string(replica['is_session_expired'])),
            ]
            tags.extend(self._tags)

            self.gauge('table.replicated.part.future', replica['future_parts'], tags=tags)
            self.gauge('table.replicated.part.suspect', replica['parts_to_check'], tags=tags)
            self.gauge('table.replicated.version', replica['columns_version'], tags=tags)
            self.gauge('table.replicated.queue.size', replica['queue_size'], tags=tags)
            self.gauge('table.replicated.queue.insert', replica['inserts_in_queue'], tags=tags)
            self.gauge('table.replicated.queue.merge', replica['merges_in_queue'], tags=tags)
            self.gauge('table.replicated.log.max', replica['log_max_index'], tags=tags)
            self.gauge('table.replicated.log.pointer', replica['log_pointer'], tags=tags)
            self.gauge('table.replicated.total', replica['total_replicas'], tags=tags)
            self.gauge('table.replicated.active', replica['active_replicas'], tags=tags)
    ...
    def query_custom(self):
        for custom_query in self._custom_queries:
            ...
            try:
                rows = self.execute_query_raw(query, 'custom query', stream=True)
